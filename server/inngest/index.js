...
// Send email to admin, if admin doesn't tale action on leave within 24 hours 
...
const attendanceReminderCron = inngest.createFunction(
    ...
    async ({ step }) =>{
        ...
        const today = await step.run("get-today-date", ()=>{
            const startUTC = new Date(new Date().toLocaleDateString("en-CA", {timezone: "Asia/Koltara"}) + "T00: 00: 00 + 05:30 * 1000");
        ...
        const onLeaveIds = await step.run("get-on-leaves-ids", async ()=>{
        ...
        // Step 4: Get employess IDS who already checked in today 
        const checkedInIds = await step.run("get-checked-ids", async ()=>{
        ...
        // Step 5: Filter absent employees (not on leave & not check in )
        ...
        return { totalActive: activeEmployees.length, onLeaves: onLeaveIds.length, checkedIn: checkedInIds.length, absent: absentEmployees.length}
    }
)
