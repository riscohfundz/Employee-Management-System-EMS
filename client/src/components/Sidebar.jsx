...
const Sidebar = () => {
  ...
  const [mobileOpen, setMobileOpen ] =useState(false)
    ...
    const sidebarContent = (
      ...
      {/* Navigation list */ }
      <div className="flex-2 px-3 space-y-0.5 overflow-y-auto">
  {navItems.map((item) => {
    ...
    return (
      ...
      <div className="p-3 border-t border-white/6">
         <button onClick={handleLogout} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/6 transition-all duration-150">
  ...
  return (
    <>
      ...
      {/* sidebar - mobile */}
      <aside className={`lg:hidden fixed inset-y left-0 w-72 bg-linear-to-b from-slate-900 via-slate-950 text-white z-50 flex flex-col transform duration-300 ${mobileOpen ? "translate-x-0" : "translate-x-full "}` }>
        {sidebarContent}
      </aside>
